...
```{r}
#AB this should have been done for datab (all data)
...
```{r}
#Null hypothesis: There is no effect of rain amount of wine quality.

lm_rq <- lm(rain ~ quality, data = rain_quality) #linear model

anova_rq<- anova(lm_rq) #type I anova

summary(anova_rq) #AB This is probably not what ou intended?? summary(lm_rq)? 
```
...
stats_table2 <- table(stats_for_table) #AB this is wrong
...
st_df <-as.data.frame(stats_table) #AB you should have used stats_for_table
