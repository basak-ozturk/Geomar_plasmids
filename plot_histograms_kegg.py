...
def plot_histograms(tsv_file, output_dir):
    ...
    # Load the TSV file
    df = pd.read_csv(tsv_file, sep="\t", header=None, names=["Cluster", "COG", "Percentage", "Name"])

    # Convert data types
    df["Name"] = df["Name"].astype(str)  # Ensure pathway names are strings
    ...
    for cluster, cluster_df in df.groupby("Cluster"):
        ...
        plt.barh(cluster_df["Name"], cluster_df["Percentage"], color="skyblue")  # Use Pathway names

        plt.xlabel("Percentage")
        plt.ylabel("Function")
        plt.title(f"COG Distribution for Cluster {cluster}")
...
# Example usage
plot_histograms("C:\\Users\\hayat\\Downloads\\R_files\\data\\eggnog_all_plasmids_COG_per_plasmid.tsv", 
                "C:\\Users\\hayat\\Downloads\\R_files\\graphs\\COG_names_per_cluster")
